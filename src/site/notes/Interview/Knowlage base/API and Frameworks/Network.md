---
{"dg-publish":true,"permalink":"/interview/knowlage-base/api-and-frameworks/network/"}
---
...
**Кратко:**  
Это современный высокопроизводительный фреймворк для RPC (Remote Procedure Call). Использует **Protocol Buffers (proto3)** как формат сериализации и **HTTP/2** как транспорт.

**Типовой вопрос:**  
Почему gRPC быстрее REST и в каких случаях его стоит предпочесть?

**Ответ:**  
- **Бинарный формат:** Protobuf упаковывает данные плотнее, чем текстовый JSON, что экономит трафик и CPU на парсинге.
- **HTTP/2:** Поддерживает мультиплексирование (много запросов в одном соединении) и стриминг (Client, Server, Bi-directional).
- **Строгая типизация:** Контракт (proto-файл) гарантирует, что клиент и сервер понимают друг друга.
- **Использование:** Идеально для внутренней связи между микросервисами (East-West traffic).
