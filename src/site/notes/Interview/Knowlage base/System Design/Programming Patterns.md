---
{"dg-publish":true,"permalink":"/interview/knowlage-base/system-design/programming-patterns/"}
---

## Базовые принципы проектирования (SOLID, DRY, KISS)

**Кратко:** 
Это набор правил, направленных на снижение сложности кода и повышение его расширяемости. Они помогают избежать «хрупкости» системы, когда изменение в одном месте ломает всё остальное, и делают кодовую базу понятной для команды.

**Типовой вопрос:** Какие базовые принципы проектирования вы знаете?

**Ответ:**
#### 1. SOLID
- **S (Single Responsibility):** Один класс/модуль — одна задача. Если класс занимается и логикой, и отправкой email, его нужно разделить.
    
- **O (Open/Closed):** Код должен быть открыт для расширения, но закрыт для изменения. Мы добавляем новый функционал через наследование или интерфейсы, не трогая старый протестированный код.
    
- **L (Liskov Substitution):** Наследник не должен ломать логику родителя. Вызов функции с объектом подкласса не должен приводить к неожиданному поведению.
    
- **I (Interface Segregation):** Много узких интерфейсов лучше, чем один «жирный». Класс не должен зависеть от методов, которые он не использует.
    
- **D (Dependency Inversion):** Модули верхних уровней не должны зависеть от нижних. Оба должны зависеть от абстракций (интерфейсов). Это позволяет легко подменять реальную БД на «заглушку» (Mock) в тестах.
    
#### 2. Принципы простоты

- **KISS (Keep It Simple, Stupid):** Не усложняй. Если задачу можно решить простым циклом, не нужно городить фабрику фабрик. Простота — залог отсутствия багов.
    
- **DRY (Don't Repeat Yourself):** Избегай дублирования. Каждое знание в системе должно иметь единственное представление. Если логика повторяется, выноси её в функцию или класс.
    
- **YAGNI (You Ain't Gonna Need It):** Тебе это не понадобится. Не пиши код для фич, которые «возможно понадобятся в будущем». Это лишняя поддержка и сложность.
