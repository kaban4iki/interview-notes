...
**Кратко:** 
Каждая переменная в Python — это не сама коробка с данными, а **указатель** на структуру в памяти. Самая базовая структура для любого объекта — это `PyObject`. Если объект может менять свой размер, он использует структуру `PyVarObject`
...
**Кратко:** 
По умолчанию Python хранит атрибуты объектов в словаре `__dict__`, что гибко, но потребляет много памяти. `__slots__` позволяет зафиксировать список атрибутов, убирая `__dict__`.

**Типовой вопрос:** 
Зачем использовать `__slots__` в классе и какие у этого подхода ограничения?

**Ответ:**
- **Экономия:** RAM потребляется на 40–50% меньше, так как данные хранятся в компактном массиве, а не в хеш-таблице.
- **Скорость:** Доступ к атрибутам становится чуть быстрее.
- **Ограничения:** Нельзя динамически добавлять новые атрибуты, которых нет в `__slots__`. Ломает множественное наследование, если оно не спроектировано аккуратно. Используется для объектов, которых в системе миллионы (например, точки координат).

---

## Интернирование (Кэширование) в Python

**Кратко:** 
Чтобы не плодить дубликаты одинаковых неизменяемых объектов (immutable), CPython переиспользует их из глобального кэша.

**Типовой вопрос:** 
Что интернируется по-умолчанию и как специально интернировать что-то?

**Ответ:**
- При старте интерпретатора CPython сразу создает в памяти объекты для чисел в диапазоне **от -5 до 256 включительно**.
- Python автоматически интернирует **строки**, которые похожи на валидные идентификаторы языка (состоят только из латинских букв, цифр и знаков подчеркивания — то, чем можно назвать переменную).
- Принудительно интернировать любую динамическую строку можно с помощью функции `sys.intern()`
